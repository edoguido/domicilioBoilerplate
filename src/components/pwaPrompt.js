...
const Prompt = ({ visible, closePopup }) => (
   <div
      style={stylePrompt}
      class={`fixed p-2 w-11/12 bottom-0 rounded-lg backdrop max-w-md border border-gray-500 ${
         visible ? "" : "hidden"
      }`}
   >
      ...
      <p class="border-b border-gray-500 py-4 px-2">
         Aggiungimi alla Home per utilizzarmi in fullscreen e offline. Così
         appena vorrai ordinare mi avrai a portata di tap!
      </p>
...
export const PWAPrompt = (props) => {
   ...
   return (
      visibilityCheks() && (
         <Prompt visible={isVisible} closePopup={handleClosePopup} {...props} />
      )
   );
};
